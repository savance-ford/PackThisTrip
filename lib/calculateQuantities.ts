...
export function calculateClothingQuantities(tripConfig: TripConfig): QuantityMap {
  ...
  let effectiveDays = Math.min(Math.max(durationDays, 1), baseCap);
  ...
  const quantities: QuantityMap = {
    ...
    pants: Math.max(1, Math.ceil(effectiveDays / 2)) * multiplier,
  ...
  if (hotWeather || isBeach) {
    quantities.shorts = Math.max(1, Math.ceil(effectiveDays / 2)) * multiplier;
    quantities["breathable-shirts"] = Math.max(1, Math.ceil(effectiveDays / 2)) * multiplier;
  }
